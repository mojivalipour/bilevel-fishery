...
def plot_resource_dynamics(
    trajectories: list[dict[str, Any]],
    title: str = "Resource Dynamics",
    ...
    ax1.set_title(f"{title} - Water Level", fontsize=14)
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        ax2.plot(
            data["steps"],
            data.get("rewards", [None] * len(data["steps"])),
            color=colors[i],
            alpha=0.7,
            linewidth=1.5,
            label=f"Episode {ep}",
    ...
    ax2.set_title(f"{title} - Reward", fontsize=14)
...
def plot_combined_trial_analysis(
    ...
    """Comprehensive visualization for water trials.

    Top-left: water level with sustainability threshold.
    Top-right: reward time series.
    Bottom-left: harvest vs quota (if present) or reward time series.
    Bottom-right: phase plot (reward vs water) or water trajectory.
    """
    if not trajectories:
        raise ValueError("No trajectory data provided")
    ...
    axes[0, 0].set_ylabel("Water Level", fontsize=12)
    axes[0, 0].set_title("Water Level Dynamics", fontsize=14)
    ...
    axes[0, 1].set_title("Reward Dynamics", fontsize=14)
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        if data.get("rewards"):
            ...
            scatter = axes[1, 1].scatter(
                range(len(data["water"])),
                data["water"],
                c=range(len(data["water"])),
                cmap="viridis",
                alpha=0.6,
                s=30,
                label=f"Episode {ep}",
    ...
    axes[1, 1].set_xlabel("Reward / Time", fontsize=12)
    axes[1, 1].set_ylabel("Water Level", fontsize=12)
...
def _group_by_episode(trajectories: list[dict[str, Any]]) -> dict[int, dict[str, list]]:
    ...
    for record in trajectories:
        ...
        if ep not in episodes:
            episodes[ep] = {
                "steps": [],
                # canonical keys used by plotting code (water, rewards)
                "water": [],
                "rewards": [],
                "harvest": [],
                "quota": [],
        ...
        if "rewards" in record:
            episodes[ep]["rewards"].append(record["rewards"])
        elif "reward" in record:
            episodes[ep]["rewards"].append(record["reward"])
        else:
            episodes[ep]["rewards"].append(None)
